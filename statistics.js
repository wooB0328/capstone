...
class LineChart extends React.Component {
  ...
  render() {
    ...
    const lines = this.state.data.map((point, index) => (
      <React.Fragment key={index}>
        <Line
          ...
          y1={this.state.svgHeight - (point.y / this.state.maxY) * this.state.svgHeight}
          x2={(this.state.svgWidth / (this.state.data.length - 1)) * (index + 1)}
          y2={
            this.state.svgHeight -
            (this.state.data[index + 1] ? (this.state.data[index + 1].y / this.state.maxY) * this.state.svgHeight : 0)
          }
          stroke="blue"
          strokeWidth="2"
        />
        ...
        <SvgText
          ...
          y={this.state.svgHeight - (point.y / this.state.maxY) * this.state.svgHeight}
          fontSize="12"
          fill="black"
        >
        ...
        <Rect
          ...
          y={this.state.svgHeight - (point.y / this.state.maxY) * this.state.svgHeight - 4} // y 좌표 조정
          width="8"  // 네모의 가로 길이
          height="8" // 네모의 세로 길이
          fill="black" // 채우기 색상
        />
    ...
    const verticalAxis = (
      <View>
        <Line x1={0} y1={0} x2={0} y2={this.state.svgHeight} stroke="black" strokeWidth="1" />
        {[50, 100].map((value, i) => {
          const yPos = this.state.svgHeight - (this.state.svgHeight / this.state.maxY) * value;
          return (
    ...
    return (
      <View style={[styles.container, { marginTop: this.state.margin }]}>
        ...
        <Text style={styles.subtitle}>회차별 점수를 시간 순으로 보여줍니다.</Text>
        <Svg height={this.state.svgHeight + this.state.margin} width={this.state.svgWidth - 10}>
          {lines}
          {horizontalAxis}
          {verticalAxis}
        </Svg>
...
const BarChart = ({ data }) => {
  ...
  // 가로축에 표시할 레이블 배열
  const horizontalAxisLabels = ['전삼국', '삼국', '남북국', '후삼국', '고려', '조선', '개항기', '일제강점기', '해방이후'];
  const horizontalAxisLabels2 = ['문화', '유물', '사건', '인물', '장소', '그림', '제도', '기구', '조약', '단체']
  if (data.length == 9) {
  ...
  return (
    <View>
      <Svg height={chartHeight} width={chartWidth}>
        {/* 가로축 */}
        <Line x1={0} y1={chartHeight} x2={chartWidth} y2={chartHeight} stroke="black" strokeWidth="3" />

        {/* 세로축 */}
        <Line x1={0} y1={0} x2={0} y2={verticalAxisHeight} stroke="black" strokeWidth="3" />
        ...
        {data.map((value, index) => (
          <React.Fragment key={index}>
            ...
            {/* 가로축 눈금 */}
            <SvgText x={index * (barWidth + 10) + barWidth / 2} y={chartHeight - 15} fontSize="12" fill="black" textAnchor="middle">
              {list[index]}
            </SvgText>
            <SvgText x={index * (barWidth + 10) + barWidth / 2} y={chartHeight - 5} fontSize="12" fill="black" textAnchor="middle">
              {data[index]}
            </SvgText>
          </React.Fragment>
        ))}
...
const Statistics = () => {
  ...
  const imageurl = "https://firebasestorage.googleapis.com/v0/b/capstone-ac206.appspot.com/o/%ED%86%B5%EA%B3%84%EB%B0%B0%EA%B2%BD%EA%B2%BD.jpg?alt=media&token=0bbb3935-6ca6-4eba-8093-65771dcbb7f0"
  const time = 302
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.answerContainer}>
        <Text style={styles.answerText}>지금까지 {TrueCheck + FalseCheck}문제를 풀었어요!!</Text>
      </View>
      <View style={styles.answerContainer}>
