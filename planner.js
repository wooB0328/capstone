...
export default function Planner() {
  ...
  const min = data.reduce((prev, current) => (prev.progress < current.progress ? prev : current)).category;
  const minIndex = data.findIndex(d => d.progress === Math.min(...data.map(d => d.progress)));
  return (
    <ScrollView>
      <View style={styles.titlecontainer}>
        ...
        <Text style={styles.normaltext}><Text style={styles.boldText}>{min}</Text> 부분이 다른 학습에 비해 부족합니다.</Text>
      ...
      <View>
        ...
        <View style={styles.buttonContainer}>
          ...
          <TouchableOpacity
            style={styles.button}
            onPress={() => {
              console.log(`"${studycategory}" 공부하러 가기 버튼이 눌렸습니다.`); //화면띄우기 추가
            }}
          >
            <Text style={styles.buttonText}>{`${studycategory} 공부하러 가기`}</Text>
          </TouchableOpacity>
        </View>
